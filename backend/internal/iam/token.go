// Session token generation and lifecycle (migrated from internal/auth with
// identical reviewed parameters; auth keeps its copy until the final phase
// deletes the legacy stack).
package iam
...
//
// Lifecycle rule (data-model.md): a session is valid while
