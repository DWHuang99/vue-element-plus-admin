// Argon2id password hashing (migrated from internal/auth with identical
// reviewed parameters; auth keeps its copy until the final phase deletes the
// legacy stack). Passwords exist only inside the current HTTP request; the
// hash never leaves the service and never appears in logs or event payloads.
package iam
